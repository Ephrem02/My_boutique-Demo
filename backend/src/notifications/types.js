...
const TYPES = {
  ...
  SUPPLIER_RETURN_RECORDED: {
    category: 'purchasing', severity: 'info', permission: 'supplier_payments.view',
    roles: ['store_manager'], inApp: true, email: false,
    vars: ['supplier_name', 'delivery_id', 'value_rwf', 'reason', 'actor_name'],
    title: 'Goods returned to {{supplier_name}}',
    body: '{{actor_name}} returned goods worth {{value_rwf}} from delivery #{{delivery_id}} to {{supplier_name}}. Reason: {{reason}}.',
  },
  CUSTOMER_PAYMENT_OVERDUE: {
    category: 'sales', severity: 'warning', permission: 'institution_payments.view',
    roles: ['store_manager'], inApp: true, email: true,
    vars: ['customer_name', 'order_id', 'balance_rwf', 'due_date'],
    title: 'Overdue customer invoice: {{customer_name}}',
    body: 'Invoice #{{order_id}} for {{customer_name}} was due on {{due_date}}; {{balance_rwf}} is still unpaid.',
  },
  CUSTOMER_RETURN_APPROVAL_NEEDED: {
    category: 'sales', severity: 'warning', permission: 'customer_returns.approve',
    roles: ['store_manager'], inApp: true, email: false,
    vars: ['customer_name', 'order_id', 'value_rwf', 'reason', 'actor_name'],
    title: 'Customer return needs approval',
    body: '{{actor_name}} recorded a return of {{value_rwf}} from {{customer_name}} (invoice #{{order_id}}). Reason: {{reason}}.',
  },
  CUSTOMER_RETURN_DECIDED: {
    category: 'sales', severity: 'info', permission: 'customer_returns.manage', mandatory: true, targetAlways: true,
    roles: [], inApp: true, email: false,
    vars: ['customer_name', 'order_id', 'decision', 'reviewer_name', 'note'],
  ...
  LEDGER_REVERSAL: {
    ...
    roles: ['store_manager'], inApp: true, email: true,
    vars: ['party_type', 'party_name', 'txn_type', 'amount_rwf', 'reason', 'actor_name'],
    title: 'Financial record reversed: {{party_name}}',
    body: '{{actor_name}} reversed a {{txn_type}} of {{amount_rwf}} for {{party_type}} {{party_name}}. Reason: {{reason}}',
  },
  // ---- Daily business cycle ----
  OPENING_REQUESTED: {
    category: 'operations', severity: 'warning', permission: 'day.open.review',
    roles: ['store_manager'], inApp: true, email: false,
    vars: ['business_date', 'actor_name', 'reason', 'opening_float_rwf'],
    title: 'Business day opening requested',
    body: '{{actor_name}} asked to open the business day {{business_date}} with {{opening_float_rwf}} in the drawer. Reason: {{reason}}',
  },
  OPENING_REQUEST_DECIDED: {
    category: 'operations', severity: 'info', permission: 'day.open.request', mandatory: true, targetAlways: true,
    roles: [], inApp: true, email: false,
    vars: ['business_date', 'decision', 'reviewer_name', 'comment'],
    title: 'Opening request {{decision}}',
    body: '{{reviewer_name}} {{decision}} your request to open the business day {{business_date}}. {{comment}}',
  },
  CLOSING_REMINDER: {
    category: 'operations', severity: 'warning', permission: 'day.close',
    roles: ['cashier', 'store_keeper', 'store_manager'], inApp: true, email: false,

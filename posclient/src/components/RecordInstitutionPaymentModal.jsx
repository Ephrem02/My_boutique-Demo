...
export default function RecordInstitutionPaymentModal({ order, onClose, onRecorded }) {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try {
      const { data } = await client.post(`/institution-orders/${order.id}/payments`, {
        amount: Number(amount),
        paid_date: paidDate,
        method,
      });
      onRecorded(data.order);
    } catch (err) {
